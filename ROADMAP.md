# 🗺️ Feuille de Route & Roadmap — Organisation KaïroOS

Bienvenue sur la feuille de route officielle et centralisée de l'écosystème **KaïroOS**, le frontend d'arcade et de salon open source sous Windows et multi-systèmes.

> 📌 **Ce document est la source unique de vérité de la Roadmap** pour l'ensemble des dépôts de l'organisation [`KairoOS-Official`](https://github.com/KairoOS-Official) et du site web officiel.
...
- [ ] **Boutique de Plugins & Extensions Universelles (JSON stdin/stdout)** :
  - Système d'extensions tierces isolées par processus (lecteur de musique chiptune, widgets météo, scraping de sources alternatives via protocole `kairo-plugin-host-v1`).
...
KaïroOS est structurellement prêt pour la portabilité grâce à l'architecture Tauri 2 et la séparation complète backend/frontend.
...
- [ ] **Système de chemins (`kairo-core/src/paths.rs`)** : Mapper `%APPDATA%` vers `$XDG_CONFIG_HOME/kairo-os` (Linux) ou `~/Library/Application Support/kairo-os` (macOS).
- [ ] **Remplacement de `windows-sys` (GetAsyncKeyState / SendInput)** : Utiliser le crate `rdev` (cross-platform).
- [ ] **Noms d'exécutables émulateurs (`config/emulators.json`)** : Mapping par OS (`retroarch.exe` → `retroarch`).

#### P1 — Important
- [ ] **Ouverture de dossiers (`src-tauri/src/commands.rs`)** : Remplacer `explorer` par `xdg-open` (Linux) / `open` (macOS).
- [ ] **Gestion ZIP sans PowerShell** : Remplacer PowerShell par les crates Rust `zip` + `reqwest`.
- [ ] **Détection Node.js universelle** : Utiliser `std::process::Command::new("node")` ou `which node`.

#### P2 — Secondaire
- [ ] **Scripts de build (`scripts/build-portable.mjs`)** : Packages `.deb` / AppImage (Linux) et `.dmg` (macOS).
- [ ] **Configuration émulateurs multi-OS** : Profils par plateforme dans `emulators.json`.
...
| Dépôt | Description |
|---|---|
| [`KairoOS`](https://github.com/KairoOS-Official/KairoOS) | Application principale (Tauri 2 + Rust + React 19) |
| [`kairos-plugins`](https://github.com/KairoOS-Official/kairos-plugins) | Catalogue officiel & communautaire d'extensions |
| [`kairos-themes`](https://github.com/KairoOS-Official/kairos-themes) | Catalogue officiel & communautaire de thèmes |
| [`kairos-website`](https://github.com/KairoOS-Official/kairos-website) | Site web officiel & API communautaire |
| [`.github`](https://github.com/KairoOS-Official/.github) | Organisation, documentation globale & roadmap |
